...
module.exports = {
    ...
    async getInput() {
        ...
        await this.interaction.channel.awaitMessages({
            ...
            if (message.content.toLowerCase() === "edit" || message.content.toLowerCase() === "delete") {
                ...
                for (let reactionRole of this.reactionRoleData.reactRoles) {
                    // Fetch the channel
                    let roleChannel = await this.interaction.guild.channels.fetch(reactionRole.channelID);
                    let roleEmoji = [];

                    // Iterate through roles
                    for (let role of reactionRole.roles) {
                        // Fetch the emoji of the role
                        let emoji = await this.interaction.guild.emojis.fetch(role.emoji);
    ...
    async confirmDeletion() {
        ...
        await this.interaction.channel.awaitMessages({
            ...
            if (["yes", "y"].includes(message.content.toLowerCase())) {
                ...
                // And save the file
                let rrID = this.reactionRoleData[this.reactionRoleIndex].id;
    ...
    async embedRoleInputPing(updateReply=true) {
        ...
        await this.interaction.channel.awaitMessages({
            ...
            let roleID = message.content.match(/<@&(\d+)>/)[1];

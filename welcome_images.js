...
module.exports = {
  async sendWelcomeImage (context, isTest = false) {
    ...
    const welcomeImages = fs.readdirSync('./data/images/welcome_images').filter(file => file.endsWith('.png'));
    ...
    const members = context.guild.members.cache.filter(member => !member.user.bot);
    const memberCount = members.size;
    const i = memberCount % 10;
    const j = memberCount % 100;
